...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		whiptail --title "WARNING" --yes-button "CONTINUE" \
			--no-button "No wait..." \
			--yesno "The user \`$name\` already exists on this system. Jax|Tech can install for a user already existing, but it will OVERWRITE any conflicting settings/dotfiles on the user account.\\n\\nJax|Tech will NOT overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also this will change $name's password to the one you just gave." 14 70
}
...
maininstall() {
	...
	whiptail --title "Jax|Tech Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 9 70
	installpkg "$1"
}
...
gitmakeinstall() {
	...
	whiptail --title "Jax|Tech Installation" \
...
makeuserjs(){
	...
	userjs="$pdir/user.js"
	ln -fs "/home/$name/.config/firefox/jaxtech.js" "$overrides"
...
for x in curl ca-certificates base-devel git ntp zsh dash; do
	whiptail --title "Jax|Tech's Installation" \
...
trap 'rm -f /etc/sudoers.d/jaxtech-temp' HUP INT QUIT TERM PWR EXIT
echo "%wheel ALL=(ALL) NOPASSWD: ALL
Defaults:%wheel,root runcwd=*" >/etc/sudoers.d/jaxtech-temp
...
echo "%wheel ALL=(ALL:ALL) ALL" >/etc/sudoers.d/00-jaxtech-wheel-can-sudo
echo "%wheel ALL=(ALL:ALL) NOPASSWD: /usr/bin/shutdown,/usr/bin/reboot,/usr/bin/systemctl suspend,/usr/bin/wifi-menu,/usr/bin/mount,/usr/bin/umount,/usr/bin/pacman -Syu,/usr/bin/pacman -Syyu,/usr/bin/pacman -Syyu --noconfirm,/usr/bin/loadkeys,/usr/bin/pacman -Syyuw --noconfirm,/usr/bin/pacman -S -y --config /etc/pacman.conf --,/usr/bin/pacman -S -y -u --config /etc/pacman.conf --" >/etc/sudoers.d/01-jaxtech-cmds-without-password
echo "Defaults editor=/usr/bin/nvim" >/etc/sudoers.d/02-jaxtech-visudo-editor
